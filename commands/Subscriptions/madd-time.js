...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const showTimeSelection = async (code, interaction = null) => {
	      ...
	      if (!entry) {
	        const msg = "**Subscription Not Found**\nلا يوجد اشتراك بهذا الايدي.";
	        return interaction ? interaction.update({ content: msg, components: [] }) : message.reply(msg);
	      }
	
	      const embed = new EmbedBuilder()
	        .setTitle('Add Subscription Time')
	        .setDescription(`اختر المدة التي تريد إضافتها للاشتراك \`${code}\`.`)
	        .addFields(
	          { name: 'User', value: `<@${entry.user}>`, inline: true },
	          { name: 'Current Expiry', value: `<t:${Math.floor(entry.expirationTime/1000)}:R>`, inline: true }
	        )
      ...
      const row1 = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder().setCustomId(`addtime_90d_${code}_${mid}`).setLabel('+90d').setStyle(ButtonStyle.Secondary)
	      ...
	      const row2 = new ActionRowBuilder().addComponents(
	        new ButtonBuilder().setCustomId(`addtime_custom_${code}_${mid}`).setLabel('Custom').setStyle(ButtonStyle.Primary),
	        new ButtonBuilder().setCustomId(`addtime_cancel_${mid}`).setLabel('Cancel').setStyle(ButtonStyle.Danger)
	      );
      ...
      const collector = (interaction ? interaction.message : prompt).createMessageComponentCollector({ filter: i => i.user.id === message.author.id, time: 60000 });
        ...
        if (i.customId.startsWith('addtime_')) {
          ...
          let durationMs = 0;
	          ...
	          if (durationStr === 'custom') {
	            const modal = new ModalBuilder().setCustomId(`modal_time_${mid}`).setTitle('Custom Time');
            ...
            try {
              ...
              durationMs = ms(durationStr);
	              if (!durationMs || durationMs <= 0) return mSubmit.reply({ content: 'الوقت غير صحيح.', flags: MessageFlags.Ephemeral });
	    ...
	    if (!selectedCode) {
	      if (timeData.length === 0) return message.reply("**No Subscriptions**\nلا توجد اشتراكات.");
	      const select = new StringSelectMenuBuilder().setCustomId(`select_time_${mid}`).setPlaceholder('Select subscription').addOptions(timeData.slice(0, 25).map(e => ({ label: e.code, value: e.code })));
	      const prompt = await message.reply({ content: '**Add Subscription Time**\nاختر الاشتراك المراد تحديث وقته:', components: [new ActionRowBuilder().addComponents(select)] });
  ...
  try {
    ...
    store.set('time', timeArray);
	    ...
	    const embed = new EmbedBuilder()
	      ...
	      .setDescription('تمت إضافة الوقت للاشتراك وتحديث تاريخ الانتهاء.')
	      .addFields(
	        { name: 'Subscription ID', value: `\`${code}\``, inline: true },
	        { name: 'Added Time', value: `\`${durationStr}\``, inline: true },
	        { name: 'New Expiry', value: `<t:${Math.floor(entry.expirationTime/1000)}:F>`, inline: false }
	      )
	      .setColor(getEmbedColor(client));
	    ...
	    if (logChannel) {
	      logChannel.send({
	        embeds: [new EmbedBuilder()
	          .setTitle('Subscription Time Updated')
	          .setDescription('تم تحديث وقت اشتراك من خلال أحد الأونرات.')
	          .addFields(
	            { name: 'User', value: `<@${entry.user}>`, inline: true },
	            { name: 'Subscription ID', value: `\`${code}\``, inline: true },
	            { name: 'Added Time', value: `\`${durationStr}\``, inline: true },
	            { name: 'Previous Expiry', value: `<t:${Math.floor(oldExpiry/1000)}:R>`, inline: true },
	            { name: 'New Expiry', value: `<t:${Math.floor(entry.expirationTime/1000)}:R>`, inline: true },
